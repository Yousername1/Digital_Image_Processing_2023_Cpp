...
{
	Filters::outputImg = Mat::zeros(inputImg.size(), CV_8UC1);
	cv::absdiff(gauss3(inputImg), gauss5(inputImg), outputImg);
	
	return outputImg * 10;
}

Mat Filters::sobel(const Mat& inputImg)
{
	Filters::outputImg = Mat::zeros(inputImg.size(), CV_8UC1);
	int Gx[3][3] = { {-1 ,0, 1},
					 {-2, 0, 2},
	...
	for (int i = 1; i < inputImg.cols - 1; i++) {
		for (int j = 1; j < inputImg.rows - 1; j++) {
			...
			for (int ii = -1; ii <= 1; ii++) {
				for (int jj = -1; jj <= 1; jj++) {
					uchar pix = inputImg.at<uchar>(j + jj, i + ii);
					convy += Gy[ii + 1][jj + 1] * pix;
					convy += Gx[ii + 1][jj + 1] * pix;
				}
			}
